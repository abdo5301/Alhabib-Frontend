<template>
  <div v-if="article_data"
    ...
    <img v-if="article_data.image" :src="article_data.image" class="w-full h-[394px] object-cover rounded-lg"
    ...
    <div class="relative flex flex-col justify-center items-center lg:gap-8 gap-6 pb-8 px-8 max-w-full">
      <span dir="ltr" v-if="article_data.created_at" class="text-gray-500 lg:text-[20px] text-[14px] font-bold leading-6">
      ...
      <div class="w-full flex-row justify-end items-center">
        <NuxtLink class="flex lg:gap-5 gap-4 pt-2 justify-end items-center" :to="localePath('/blog/' + article_data.id)">
