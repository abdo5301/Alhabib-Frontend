<template>
  ...
  <div @click="modal.show()"
    class="w-full shadow lg:shadow-none lg:mt-[30px] mt-5 h-[70px] lg:h-[92px] flex items-center justify-between px-5 lg:px-1 lg:rtl:pr-4 lg:rtl:pl-[11px] lg:ltr:pl-4 lg:ltr:pr-[11px]  flex-shrink-0 bg-white rounded-lg lg:rounded-md lg:ring-1 ring-gray-400 cursor-pointer">
    <div class="flex lg:gap-[17px] gap-3 items-center justify-start">
      <span>
        <svg xmlns="http://www.w3.org/2000/svg" class="lg:w-[55px] w-[36px] lg:h-[56px] h-[36px]" viewBox="0 0 45 46" fill="none">
          <path
      ...
      <div class="flex flex-col gap-3 text-black leading-5">
        <h4 class="lg:font-bold font-semibold lg:text-2xl text-sm">{{ $t('cart_receive_form_shop_title') }}</h4>
        <span class="hidden font-medium lg:text-xl text-sm">{{ $t('product_receive_from_shop_content') }}</span>
      </div>
    </div>
    <span class="lg:rtl:ml-[37px] lg:ltr:mr-[37px]">
