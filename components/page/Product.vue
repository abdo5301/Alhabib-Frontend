      ...
      <div
        ...
        <div
          ...
          <div v-show="!data_loader" class="w-full gap-2 lg:gap-5 relative py-4 flex justify-between items-center">
            ...
            <ClientOnly>
              <!-- cart btn -->
              <button v-if="!data_loader" type="button" v-show="!out_stock" @click="addToCart()"
          ...
          <div v-if="!data_loader" class="w-full bg-[#F9FAFB] max-h-[300px] overflow-y-auto no-scrollbar"
...
onMounted(async () => {
  ...
  try {
    product_fetch_data.value = await useNuxtApp().$apiFetch('/master-products/get?master_product_q=' + url_product_id)
  } catch (error) {
