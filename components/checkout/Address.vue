<template>
  ...
  <div class="flex flex-col lg:gap-[30px] lg:py-[30px] pt-[10px]">
    ...
    <div v-if="(step == 'select_address' || step == 'select_payment')"
      ...
      <div class="lg:p-[17px] flex justify-start">
        ...
        <div class="flex lg:flex-row flex-col justify-start items-start flex-1 gap-4 text-gray-500 leading-5">
          ...
          <div class="flex flex-col gap-2 font-normal text-sm">
            <span v-if="current_customer_data && current_customer_data.name">{{ current_customer_data.name }}</span>
            <span v-if="current_customer_data && current_customer_data.phone" dir="ltr" class="flex justify-end">{{
              current_customer_data.phone }}</span>
            <span v-if="current_address_data && current_address_data.region && current_address_data.city">{{
              $t('text_some_region') }} {{ current_address_data.region.name }}، {{ current_address_data.city.name }}
            </span>
            <span v-if="current_address_data && current_address_data.neighborhood">{{
              current_address_data.neighborhood.name }}</span>
          </div>
        </div>
        <!-- Edit Link -->
      ...
      <!-- Shipping Time -->
      <div v-if="step == 'select_payment'" class="lg:p-[17px] flex justify-start flex-col lg:flex-row gap-2 lg:gap-5">
        <h6
    ...
    <form v-if="step == 'add_address' || step == 'edit_address'"
      ...
      <Input v-else input_style="h-[50px]" label_style="lg:text-lg pb-2" v-model="phone" :label_name="$t('label_phone')"
        label_for="input-phone" input_id="input-phone" input_name="phone" input_type="text" input_dir="ltr"
        :input_phone_country="true" input_placeholder="534398859" @phone-country="getPhoneCountry" />
      <!-- Region -->
      <InputSelect input_id="input-region-select" :select_data="region_array" :select_value="JSON.stringify(region)" v-model="region" input_style="h-[50px]"
        option_index_name="name">
        {{ $t('label_region') }}
      </InputSelect>
      <!-- City -->
      <InputSelect input_id="input-city-select" :select_data="city_array" :select_value="JSON.stringify(city)" v-model="city" input_style="h-[50px]"
        option_index_name="name">
        {{ $t('label_city') }}
      </InputSelect>
      <!-- Neighborhood -->
      <InputSelect input_id="input-neighborhood-select" :select_data="neighborhood_array" :select_value="JSON.stringify(neighborhood)" v-model="neighborhood"
        input_style="h-[50px]" option_index_name="name">
        {{ $t('label_neighborhood') }}
      </InputSelect>

      <!-- Address Description -->
      <Input input_style="h-[50px]" label_style="text-base lg:text-xl lg:pb-2 pb-1" v-model="address"
        :label_name="$t('label_address')" label_for="input-address" input_id="input-address" input_name="address"
  ...
  <!-- Shipping Time -->
  <div v-if="step != 'select_payment'" class="flex flex-col lg:gap-[30px] gap-[25px] pt-[40px] pb-[25px] lg:py-[30px]">
    <!-- Title -->
    <h4 class="flex items-center justify-start text-gray-500 lg:text-gray-900 font-bold leading-5 lg:text-xl text-base">
      {{ $t('shipping_time_title') }}
    </h4>
    <!-- Item -->
  ...
  <button v-if="step == 'select_address'" @click="$emit('nextStep', 'select_payment')"
  ...
  <button v-if="step == 'add_address' || step == 'edit_address'" type="submit" @click="submitAddress(step)"
    :disabled="!phone || !full_name"
...
const full_name = ref(props.current_customer_data && props.current_customer_data.name ? props.current_customer_data.name : '')
const phone_country = ref('+966')
const phone = ref(props.current_customer_data && props.current_customer_data.phone ? props.current_customer_data.phone : '')
const region = ref(props.current_address_data && props.current_address_data.region ? props.current_address_data.region : region_array[0])
const city = ref(props.current_address_data && props.current_address_data.city ? props.current_address_data.city : city_array[0])
const neighborhood = ref(props.current_address_data && props.current_address_data.neighborhood ? props.current_address_data.neighborhood : neighborhood_array[0])
const address = ref(props.current_address_data && props.current_address_data.address ? props.current_address_data.address : '')
...
function submitAddress(form_type) {

  if (typeof region.value !== 'object') {
    region.value = JSON.parse(region.value)
  }
  if (typeof city.value !== 'object') {
    city.value = JSON.parse(city.value)
  }
  if (typeof neighborhood.value !== 'object') {
    neighborhood.value = JSON.parse(neighborhood.value)
  }
  ...
  emits('saveAddress', new_address_data)
}
